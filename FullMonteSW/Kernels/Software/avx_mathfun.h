/* AVX implementation of sin, cos, exp, log

    Original (C) Julien Pommier 2007
    SSE1+MMX or SSE2 source code available gruntthepeon.free.fr/ssemath

    Updated for AVX instructions by Jeffrey Cassidy (C) 2014

    This is released under the zlib license as indicated below
*/ 
...
#define _PS_CONST(Name, Val)                                            \
		static const __m256 _ps_##Name = _mm256_set1_ps(Val)
//static const ALIGN16_BEG float _ps_##Name[4] ALIGN16_END = { Val, Val, Val, Val }
#define _PI32_CONST(Name, Val)                                            \
	static const __m256 _pi32_##Name = _mm256_castsi256_ps(_mm256_set1_epi32(Val))
...
static const __m256 _ps_min_norm_pos = _mm256_castsi256_ps(_mm256_set1_epi32(0x00800000));
...
static const __m256 _ps_sign_mask = _mm256_castsi256_ps(_mm256_set1_epi32(0x80000000));
//_PS_CONST_TYPE(sign_mask, int, (const int)0x80000000);
...
//#else
  ...
  // need to split emm0 ops; no AVX equivalent

  __m128i emm0lo = _mm_srli_epi32(_mm_castps_si128(_mm256_extractf128_ps(x,0)),23);
  __m128i emm0hi = _mm_srli_epi32(_mm_castps_si128(_mm256_extractf128_ps(x,1)),23);

//#endif
  /* keep only the fractional part */
  x = _mm256_and_ps(x, _ps_inv_mant_mask);
  x = _mm256_or_ps(x, _ps_0p5);

//#ifndef USE_SSE2
//  /* now e=mm0:mm1 contain the really base-2 exponent */
//  mm0 = _mm_sub_pi32(mm0, *(v2si*)_pi32_0x7f);
//  mm1 = _mm_sub_pi32(mm1, *(v2si*)_pi32_0x7f);
//  __m256 e = _mm_cvtpi32x2_ps(mm0, mm1);
//  _mm_empty(); /* bye bye mmx */
//#else

  //emm0 = _mm256_sub_epi32(emm0, _pi32_0x7f);
  emm0lo = _mm_sub_epi32(emm0lo,_mm_set1_epi32(0x7f));
...
//#endif
  ...
  */
  ...
  //__m256 mask = _mm256_cmplt_ps(x, _ps_cephes_SQRTHF);
  __m256 mask = _mm256_cmp_ps(x, _ps_cephes_SQRTHF, _CMP_LT_OQ); 	// compare ordered less-than, quiet
  ...
  __m256 y = _ps_cephes_log_p0;
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p1);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p2);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p3);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p4);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p5);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p6);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p7);
  y = _mm256_mul_ps(y, x);
  y = _mm256_add_ps(y, _ps_cephes_log_p8);
  y = _mm256_mul_ps(y, x);

  y = _mm256_mul_ps(y, z);
  

  tmp = _mm256_mul_ps(e, _ps_cephes_log_q1);
  ...
  tmp = _mm256_mul_ps(e, _ps_cephes_log_q2);
  x = _mm256_add_ps(x, y);
  x = _mm256_add_ps(x, tmp);
...
//#endif
  ...
  /* extract the sign bit (upper one) */
  sign_bit_sin = _mm256_and_ps(sign_bit_sin, _ps_sign_mask);
...
//  emm2 = _mm256_cmpeq_epi32(emm2, _mm256_setzero_ps());
  ...
  __m128i emm2lo = _mm_castps_si128(_mm256_extractf128_ps(_mm256_castsi256_ps(emm2),0));

  __m128i zero_epi32 = _mm_castps_si128(_mm_setzero_ps());

  emm2 = _mm256_castps_si256(
		  _mm256_set_m128(
				  _mm_castsi128_ps(_mm_cmpeq_epi32(emm2hi,zero_epi32)),
  ...
  y = _mm256_mul_ps(y, z);
  y = _mm256_mul_ps(y, z);
  __m256 tmp = _mm256_mul_ps(z, _ps_0p5);
  y = _mm256_sub_ps(y, tmp);
  y = _mm256_add_ps(y, _ps_1);
  
  /* Evaluate the second polynom  (Pi/4 <= x <= 0) */

  __m256 y2 = _ps_sincof_p0;
