...
/** Records memory traces:
 *
 * Tetra		tetra residency with number of absorptions (0 -> passed through)
 * Face			exit scoring
 *
 */

class MemTraceLogger : public LoggerBase
{
...
public:
    ...
    MemTraceLogger(string fnTetra_,string fnExit_) : fnTetra(fnTetra_),fnExit(fnExit_),os_tetra(fnTetra_),os_exit(fnExit_),
    MemTraceLogger(const MemTraceLogger& l_) : fnTetra(l_.fnTetra),fnExit(l_.fnExit),os_tetra(l_.fnTetra),os_exit(l_.fnExit)
...
public:
    ...
    class WorkerThread : public MemTraceLogger {
        public:
		typedef void logger_member_tag;
        WorkerThread(string a,string b) : MemTraceLogger(a,b){}
    };
