...
#include "../../../Geometry/Queries/TetrasNearPoint.hpp"
#include <FullMonteSW/Geometry/Queries/TetraEnclosingPointByLinearSearch.hpp>
...
//		cout << "Error locating tetra" << endl;
	...
	if (elHint != -1u && el.value() != elHint)
		cout << "WARNING: Element hint provided (" << elHint << ") does not match enclosing tetra found (" << el.value() << ")" << endl;

	cout << "  INFO: Point source at " << pos[0] << ',' << pos[1] << ',' << pos[2] << endl << " in tetra " << el.value() << endl;
...
{
	SSE::Point3 pos(pb->position());	// position input
...
template<class RNG>void TetraEmitterFactory<RNG>::doVisit(Source::Ball* bs)
{
	...
	if (w.size() == 0)
	{
		cout << "Radius specified is too small to fully contain any tetras." << endl;
		//cout << "Adding the one tetra containing the point." << endl;
		//w.emplace_back(TP.centreTetra(),1.0);
	}
